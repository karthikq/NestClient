...
const CustomDialog = ({ setOpen, setItems, open, items, images }) => {
  ...
  const handleClose = () => {
    ...
    if (!fileExists && images.urls.length > images.names.length) {
      const removeLastUrl = images.urls.splice(-1);
      const data = images.urls.filter((el) => el !== removeLastUrl[0]);

      setItems((preValue) => ({ ...preValue, urls: data }));
    }
  };
  return (
