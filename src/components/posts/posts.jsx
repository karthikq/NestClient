...
const Posts = ({ item, user }) => {
  ...
  useEffect(() => {
    if (postId) {
      ...
      setTimeout(() => {
        document.getElementById(postId + "post").style.display = "block";
        document.getElementById(postId + "contents").style.border =
          "2px solid #5814d683";
      }, 200);
      setTimeout(() => {
        document.getElementById(postId + "contents").style.border =
          "2px solid rgba(255, 255, 255, 0.336)";
      }, 4000);
    }
  }, [postId]);
  ...
  return (
    <div id={item?.postId} ref={ref} className="post-container">
      ...
      <div id={item.postId + "contents"} className="post-contents">
